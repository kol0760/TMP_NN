...
class MultimodalMoleculeNet_for_MSC(nn.Module):
    def __init__(self, in_channels_3d=1, in_channels_2d=1, in_features_rnn=23, 
        ...
        super(MultimodalMoleculeNet_for_MSC, self).__init__()
...
class MultimodalMoleculeNet_for_Bergman(nn.Module):
    def __init__(self, in_channels_3d=1, in_channels_2d=1, in_features_rnn=22, 
                 mid_channels=16, out_channels=32, hidden_size_rnn=128, dropout_rate=0.3):
        super(MultimodalMoleculeNet_for_Bergman, self).__init__()
        ...
        self.lstm_3d = nn.LSTM(input_size=256, hidden_size=hidden_size_rnn, num_layers=1, batch_first=True)
        self.lstm_2d = nn.LSTM(input_size=32*10, hidden_size=hidden_size_rnn, num_layers=1, batch_first=True)
        self.lstm_1d = nn.LSTM(input_size=in_features_rnn, hidden_size=hidden_size_rnn,dropout=0.0, num_layers=1, batch_first=True)
    ...
    def forward(self, x_3d, x_2d, x1d):
        batch_size, num_atoms, d, h, w = x_3d.size()
        ...
        # 3D CNN branch
        x3d = x_3d.view(batch_size * num_atoms, 1, d, h, w)  
        x3d = F.relu(self.bn3d_1(self.conv3d_1(x3d)))
        x3d = self.pool3d(x3d)
        x3d = F.relu(self.bn3d_2(self.conv3d_2(x3d)))
        ...
        # 2D CNN branch
        batch_size, num_atoms, h, w = x_2d.size()
        x2d = x_2d.view(batch_size * num_atoms, 1, h, w)  
        x2d = F.relu(self.bn2d_1(self.conv2d_1(x2d)))
        x2d = self.pool2d(x2d)
        x2d = F.relu(self.bn2d_2(self.conv2d_2(x2d)))

        x2d = x2d.view(batch_size, num_atoms, -1)  
        x2d, _ = self.lstm_2d(x2d)
        x2d = x2d[:, -1, :] 

        # RNN branch
        x1d, _ = self.lstm_1d(x1d)
        x1d = x1d[:, -1, :]  
        ...
        # Concatenate outputs from three branches
        x = self.weighted_concat(x3d, 0.8*x2d, x1d, 0.8*x3d_2d, 0.8*x1d_2d)
        # x = torch.cat((x3d, x2d, x_rnn,x3d_2d,x1d_2d), dim=1)
        x = self.layers(x)
        return x
