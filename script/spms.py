...
class SPMS():
    def __init__(self,sdf_file='',rdkit_mol=None,key_atom_num=None,sphere_radius=None,desc_n=40,desc_m=40,
        ...
        if sdf_file == '':
            mol = rdkit_mol
        elif sdf_file.split('/')[-1].split('.')[-1]=='xyz':
            mol = Chem.MolFromXYZFile(sdf_file)
        else:
...
class Calc_SPMS():
    def __init__(self, spms_acc=10, sphere_radius=4):
...
import sys
spms_calculator = Calc_SPMS(spms_acc=10, sphere_radius=7)
x2d = spms_calculator.calc_spms(sys.argv[1])
npy_filename = sys.argv[1].replace('.xyz', '_spms.npy')

np.save(npy_filename , x2d)
