...
def get_EDY_Subgraph(G: nx.classes.graph) -> np.ndarray:
    ...
    Returns:
        np.ndarray: An array of length  6 representing the enediyne scaffold.
    ...
    if len(valid_subgraphs) == 1:
        ...
        for node in valid_subgraphs:
            ...
            if [G.degree[i] for i in path_nodes]==[2, 2, 3, 3, 2, 2]:return (np.array(path_nodes))
        return False
...
def ordered_path_nodes(G: nx.classes.graph, subgraph_nodes):
    ...
    start_node = next((node for node in subgraph.nodes() if subgraph.degree(node) == 1), None)
